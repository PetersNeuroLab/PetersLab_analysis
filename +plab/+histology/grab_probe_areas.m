...
%
% probe_vector - can be either: 
%       fitted line: matrix [2 (start/end) x 3 (dim) x n (shanks)]
%       any number of points: cell array {n shanks}

% Load atlas
[av,~,st] = ap_histology.load_ccf;

% If probe points not cell array, convert
if ~iscell(probe_vector)
    probe_vector = reshape(mat2cell(probe_vector,2,3,ones(size(probe_vector,3),1)),[],1);
end

% Get areas along each shank
n_shanks = length(probe_vector);
...
for curr_shank = 1:n_shanks
    ...
    shank_sample_coords_ccf_fullrange = ...
        round(interp1([0;cumsum(sqrt(sum(diff(shank_vector,[],1).^2,2)))],shank_vector, ...
    ...
    % (signed distance relative to tip: +DV/-DV)
    ccf2mm = 1/100; % conversion factor: CCF is in 10um voxels (untransformed)

    shank_tip_distance = ccf2mm*vecnorm((shank_vector(end,:) - ...
        shank_sample_ccf_sub(shank_sample_area_boundaries,:))')';

    shank_tip_distance_sign = sign(shank_vector(end,2) - ...
        shank_sample_ccf_sub(shank_sample_area_boundaries,2));
   
    shank_tip_distance_signed = shank_tip_distance.*shank_tip_distance_sign;
