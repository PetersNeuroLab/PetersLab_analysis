...
for shank = reshape(shanks,1,[])
    ...
    mua_depth_window = 0.01; % MUA depth window (mm)
    mua_depth_smooth = 10; % Moving window to smooth MUA depth (bins)
    ...
    mua_corr_smooth = fillmissing(smoothdata2(mua_corr,'movmedian',mua_depth_smooth),'constant',0);
    ...
    clim(mua_corr_axes,[-1,1].*max(tril(abs(mua_corr_smooth),-1)*0.5,[],'all'));
