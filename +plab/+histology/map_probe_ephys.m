...
if isfield(AP_histology_processing.annotation,'ephys_path')
        [~,ephys_idx] = ismember({AP_histology_processing.annotation.ephys_path}, ...
            probe_mapping_table.UserData.ephys_paths);

        probe_mapping_table.Data(ephys_idx~=0,2) = ...
            probe_mapping_table.ColumnFormat{2}(ephys_idx(ephys_idx~=0));
        
        color_table(probe_mapping_table);
end

end

function color_table(probe_mapping_table,~)
% Color table cells based on unchosen/valid/invalid
unchosen_idx = find(strcmp(probe_mapping_table.Data(:,2),'No recording chosen'));

invalid_idx = setdiff(...
    find(sum(string(probe_mapping_table.Data(:,2)) == ...
...
% For all mapped annotations, save ephys path
for curr_annotation = find(ephys_idx ~= 0)'
    AP_histology_processing.annotation(curr_annotation).ephys_path = ...
        probe_mapping_table.UserData.ephys_paths{ephys_idx(curr_annotation)};
end
...
probe_mapping_table.Data(:,2) = {'No recording chosen'};
color_table(probe_mapping_table)
end
