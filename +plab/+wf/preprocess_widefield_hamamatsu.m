function [U,Vrec,im_avg,frame_info] = preprocess_widefield_hamamatsu(im_files,n_widefield_frames)
%
...
switch im_filetype
    ...
    case 'dcimg'
        ...
        if sum(im_file_nframes) ~= sum(n_widefield_frames)
            error('Widefield preprocessing: mismatch n frames in files and timelites')
        end
        ...
        n_frames = sum(n_widefield_frames);
        n_colors = 2;
        im_frame_color = mat2cell( ...
            1+mod((1:n_frames) - ...
            interp1(recording_start_frame_idx,recording_start_frame_idx,...
        ...
        im_rec_idx = mat2cell(cell2mat(cellfun(@(x,idx) idx*ones(1,x), ...
            num2cell(n_widefield_frames)',num2cell(1:length(n_widefield_frames)), ...
...
if n_frames ~= sum(n_widefield_frames)
...
if ~exist('n_widefield_frames','var') || isempty(n_widefield_frames)
    ...
    % Store V's as recordings x color
    Vrec = cell(length(n_widefield_frames),n_colors);
